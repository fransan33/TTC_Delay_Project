...
- **Secondary Objective 1:** Evaluate yearly per-line on-time performance against TTC's published 90% KPI target to assess whether current service levels meet the organization's own performance standard
...
- Top-N ranking per partition — RANK() and DENSE_RANK() window functions partitioned by station, time of day, or day of week in MySQL to surface the single most common delay cause per grouping.
...
- Segmentation by operational controllability — delay causes classified into within_control, partial_control, and outside_control tiers via a reference lookup table in MySQL, enabling analyses to be filtered by what TTC can realistically act on vs. can't act on.
...
- On-time performance benchmarking — per train line, OTP is calculated in a yearly basis using min_gap ≤ 8 as the on-time threshold, and is benchmarked against TTC's published KPI of 90% OTP target from the 2025 Corporate Plan Mid-Year Progress Report. 
